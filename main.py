...
def help(update, context):
    update.message.reply_text('MEVCUT KOMUTLAR:\n' +
                              ...
                              ' özel bot yapımı =>  , @MajesteSahip\n\n' +
                              ' Diger Botlar => @MajesteBotlar\n\n' +
                              ' music botu => @MajesteMusicProBot\n\n' +
                              ' 📣 Güncelleme Kanlı @MajesteBotlar', reply_to_message_id=True)
...
def cancel(update, context):
    ...
    if get_or_create_game(update.effective_chat.id):
        ...
        send_message(update, "❌ Oyun başarılı bir şekilde sonlandırıldı Özel reklam için => @MajesteSahip .")
...
def command_start(update, context: CallbackContext):
    if update.effective_chat.type == "private":
        ...
        addme = InlineKeyboardButton(text="🗯 Beni Gruba Ekle 🗯", url="https://t.me/MajesteKelimeOyunBot?startgroup=a")
        sohbet = InlineKeyboardButton(text="👨🏻‍💻 Sahip ", url="https://t.me/MajesteSahip")
        oyun = InlineKeyboardButton(text="📣Güncelleme Kanalı📣", url="https://t.me/MajesteBotlar")
        admin = InlineKeyboardButton(text="🔊 ᴍᴜᴢɪᴋ ʙᴏᴛᴜ 🔊", url="https://t.me/MajesteMusicProBot")

        keyboard = [[addme],[sohbet],[oyun],[admin]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        update.message.reply_text('ÖZEL SOHBETTE OYUN OYNANMIYOR!\n'+
                                  '**Merhaba! Botumuz @MajesteKelimeOyunBot oyunun telegrama uyarlanmış hali.Gruba ekleyip yetki verdikten sonra geriye oynamak kalıyor 😉 destek için @MajesteSahip.Komutlar için /help yazınız**!\n' +
                                  'Not =>  ❌ kanala abone olmayı unutmayınız @MajesteBotlar ❌\n', reply_to_message_id=True, reply_markup=reply_markup)
